...
const descriptionGET = (req, res) => {
  ...
  res.status(200).sendFile("ifcToDae.html", options);
};

const IfcToDaeGET = (req, res) => {
  res.set({
    Server: "My Node.js Server",
  ...
  res.status(200).json({
    message:
      "Send an IFC file to this endpoint with an HTTP POST request to convert it to a DAE file.",
    accepts: "application/ifc",
    returns: "application/collada+xml",
  });
};

const IfcToDaePOST = async (req, res, next) => {
